...
var MP4Player = (function reader() {
  ...
  constructor.prototype = {
    ...
    codecinfo: function(width, height, sps, pps) {
...
var Broadway = (function broadway() {
  function constructor(div, map, channel) {
    var started = false;
    this.canvas = document.createElement('canvas');
    //var canvas_over = document.createElement('canvas');
    this.map = map;
    ...
    //this.canvas.setAttribute('style', "z-index: 1; position: absolute; background-color: #333333;");
    ...
    /*
    canvas_over.setAttribute('style', "z-index: 2; position: absolute;"); // background-color: #FF0000;");
    //canvas_over.id = 'layer2';
    //canvas_over.zindex = 2;
    //canvas_over.position = 'absolute';
    canvas_over.width = width;
    canvas_over.height = height;
    canvas_over.left = 0;
    canvas_over.top = 0;
    //canvas_over.style.backgroundColor = "#FF0000";
    */
    ...
    this.canvas.onclick = function () {
      ...
      if(started == false) {
	started = true;
	this.webclient = new WebClient(this.channel);
      	this.webclient.init(this.player.codecinfo.bind(this.player), this.player.play2.bind(this.player), this.show_data.bind(this), this.show_geoMap.bind(this));
      	//init(this.player.codecinfo.bind(this.player), this.player.play2.bind(this.player), this.show_data.bind(this), this.show_geoMap.bind(this));
      }
      else {
    ...
    //div.appendChild(canvas_over);

    
    var controls = document.createElement('div');
    controls.setAttribute('style', "z-index: 100; position: absolute; bottom: 0px; background-color: rgba(0,0,0,0.8); height: 30px; text-align: left; width: " + width + "px; top: " + height + "px");
    ...
    this.volume.setAttribute('style', "z-index: 4; position: absolute; height: 30px; width: 30px; top: " + (this.canvas.height-30) + "px; left: " + (this.canvas.width-30) + "px;");
    var image = new Image();
    image.src = "images/sound-bar.gif";
    image.width = 25;
    image.height = 25;
    this.volume.appendChild(image);
    div.appendChild(this.volume);
    //this.volume.setAttribute('style', "display: none;");
    this.volume.style.display = "none";


    this.geo_address = document.createElement('div');
    this.geo_address.id = "link_bar";
    this.geo_address.setAttribute('style', "z-index: 4; position: absolute; height: 30px; width: 400px; top: " + (this.canvas.height-40) + "px; left: " + 10 + "px;");
    this.pre_text = document.createElement("p");
    this.pre_text.style.wordWrap = "break-word";
    this.pre_text.style.color = "white";
    this.geo_address.appendChild(this.pre_text);
    div.appendChild(this.geo_address);
    //this.volume.setAttribute('style', "display: none;");
    

    this.geo_map = document.getElementById('map-canvas');
    this.geo_map.setAttribute('style', "z-index: 4; position: absolute; height: 300px; width: 400px; top: " + (this.canvas.height-330) + "px; left: " + 30 + "px;");
    this.geo_map.style.border = '3px solid #0b0';
    div.appendChild(this.geo_map);
    this.geo_map.style.display = "none";
    this.show_map = false;


    this.battery = document.createElement('div');
    //this.battery.setAttribute('style', "z-index: 4; position: absolute; height: 30px; width: 100px; top: " + (this.canvas.height-30) + "px; left: " + (this.canvas.width-30-50) + "px;");
    this.battery.setAttribute('style', "z-index: 4; position: absolute; height: 30px; width: 100px; top: -32px; left: " + (this.canvas.width-50) + "px;");
    this.battery_img = new Image();
    this.battery_img.setAttribute('style', "position: absolute; top: 8px; left: 0px;");
    this.battery_stat = document.createElement("p");
    this.battery_stat.style.wordWrap = "break-word";
    this.battery_stat.setAttribute('style', "position: absolute; top: -10px; left: 16px; color: white;");
    this.battery.appendChild(this.battery_img);
    this.battery.appendChild(this.battery_stat);
    div.appendChild(this.battery);


    this.notice = document.createElement('div');
    //this.notice.setAttribute('style', "z-index: 4; position: absolute; height: 0px; width: 400px; top: -10px; left: " + 5 + "px;");
    this.notice.setAttribute('style', "z-index: 100; position: absolute; bottom: 0px; height: 30px; text-align: right; width: "+ width + "px; top: " + (height-12) + "px");
    this.notice_text = document.createElement("p");
    this.notice_text.style.wordWrap = "break-word";
    this.notice_text.style.color = "deepskyblue";
    this.notice.appendChild(this.notice_text);
    div.appendChild(this.notice);

    this.timestamp = document.createElement('div');
    this.timestamp.setAttribute('style', "z-index: 4; position: absolute; height: 10px; width: 120px; top: -10px; left: " + (this.canvas.width-120) + "px;");
    this.time_text = document.createElement("p");
    this.time_text.style.wordWrap = "break-word";
    this.time_text.style.color = "deepskyblue";
    this.timestamp.appendChild(this.time_text);
    div.appendChild(this.timestamp);
  ...
  constructor.prototype = {
    ...
    show_geoMap: function() {
	if(this.show_map) {
	    this.show_map = false;
    	    this.geo_map.style.display = "none";
	} else {
	    this.show_map = true;
    	    this.geo_map.style.display = "block";
	    google.maps.event.trigger(this.map, 'resize');
	    this.map.setCenter(this.geo_mapLatlng);
            infowindow.setContent(this.geo_mapAddress);
    ...
    show_data: function() {
      ...
      if(type == 1) { // volume display
	//ERROR("volume display..");
	if(value === 'ON') {
      ...
      if(type == 2) { // geo map...
    	//this.pre_text.innerHTML = "<font size=2>" + value + "</font>";
	this.geo_mapLatlng = arguments[2];
	this.geo_mapAddress = value;
	var temp = "<a href='javascript:this.__show_geoMap()'>" + value + "</a>";
	//DEBUG(temp);
    	this.pre_text.innerHTML = temp.fontsize(2);
      } else
      if(type == 3) { // battery level
	var level = parseInt(Number(value) / 10);

    	this.battery_img.width = 20;
    	this.battery_img.height = 20;
	this.battery_img.src = "images/battery_" + level + "0.png";
	var temp = value + "%";
	this.battery_stat.innerHTML = temp.fontsize(2);
      } else 
      if(type == 4) {  // notice..
	this.notice_text.innerHTML = value.fontsize(2);
      } else
      if(type == 5) { // timestamp
	var date = new Date(Number(value));
	var temp = date.getFullYear() + "/" + (date.getMonth()+1) + "/" + date.getDate() + " " +
	       date.getHours() + ":"  + date.getMinutes() + ":" + date.getSeconds();
	this.time_text.innerHTML = temp.fontsize(2);
      }
    }
  };
  return constructor;
