...
//DEBUG("BufferList.advance] this.first => " + this.first);
    if (this.first) {
...
ERROR("Stream.rewind] UnderflowError..");
      throw new AV.UnderflowError();
    }
    if (!this.list.first) {
      this.list.rewind();
...
//DEBUG("EventEmitter.off] envent=" + event);
    var index, _ref;
...
//DEBUG("EventEmitter.once] ... envent=" + event + ", fn=" + fn);
    var cb;
    return this.on(event, cb = function() {
//DEBUG("EventEmitter.once] ... envent=" + event);
      this.off(event, cb);
...
//DEBUG("EventEmitter.emit] ..." + _ref1);
    for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
      fn = _ref1[_i];
      fn.apply(this, args);
//DEBUG("EventEmitter.emit] fn.apply" + fn);
    }
  };
...
//DEBUG("Demuxer] register => " + demuxer);
...
//DEBUG("AV.Asset.start] ..");
    if (this.active) {
//DEBUG("AV.Asset.start] active");
      return;
    }
    if (decode != null) {
//DEBUG("AV.Asset.start] decode is not null");
      this.shouldDecode = decode;
    }
    if (this.shouldDecode == null) {
//DEBUG("AV.Asset.start] shouldDecode is null");
      this.shouldDecode = true;
    }
    this.active = true;
    this.source.start();
    if (this.decoder && this.shouldDecode) {
//DEBUG("decoder, shouldDecode is not null..");
      return this._decode();
    }
  };
...
//DEBUG("Asset.probe] chunk => " + chunk);
    demuxer = AV.Demuxer.find(chunk);
    if (!demuxer) {
//DEBUG("Asset.probe] demuxer is null..");
      return this.emit('error', 'A demuxer for this container was not found.');
    }
//DEBUG("Asset.probe] demuxer => " + demuxer);
...
//DEBUG("Asset.findDecoder] ...");
    ...
    if (!decoder) {
//DEBUG("Asset.findDecoder] A decoder for " + this.format.formatID + " was not found.");
    ...
    if (this.format.floatingPoint) {
      this.decoder.on('data', (function(_this) {
        return function(buffer) {
//DEBUG("Asset.findDecoder] floatingPoint.");
...
//DEBUG("Asset.findDecoder] else floatingPoint.");
...
//DEBUG("Asset.findDecoder] emit(decodeStart).");
    if (this.shouldDecode) {
//DEBUG("Asset.findDecoder] this._deocde()");
      return this._decode();
    }
  };
