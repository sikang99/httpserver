...
func main() {
	// check parameters
	sourcePtr := flag.String("source", "http://192.168.0.91/axis-cgi/mjpg/video.cgi", "source mjpg url")
	usernamePtr := flag.String("username", "imoment", "source mjpg username")
	passwordPtr := flag.String("password", "imoment", "source mjpg password")

	bindPtr := flag.String("bind", ":8080", "proxy bind address")
	urlPtr := flag.String("url", "/stream", "proxy serve url")
